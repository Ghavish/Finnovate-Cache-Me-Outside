...
const CAMERA_ERRORS = {
  NotAllowedError: k('Camera access was blocked. Allow it in your browser settings, or use your camera app below.'),
  NotFoundError: k('No camera was found on this device. Use Upload document instead.'),
  NotReadableError: k('The camera is busy in another app. Close it and try again.'),
}
...
export default function CameraCapture({ busy, onPhoto, onError }) {
  ...
  async function startCamera() {
    ...
    try {
      ...
      stopCamera()
      onError(CAMERA_ERRORS[err.name] || k('The camera could not start. Use your camera app below instead.'))
    } finally {
      setStarting(false)
    }
  }
  ...
  async function onCameraAppPhoto(event) {
    ...
    if (!file.type.startsWith('image/')) {
      onError(k('That was not a photo. Try again.'))
      return
    }
    onError('')
    const blob = await shrinkPhotoFile(file)
    if (blob.size > MAX_PHOTO_MB * 1024 * 1024) {
      onError(t('That photo is too large. The limit is {size} MB.', { size: MAX_PHOTO_MB }))
      return
    }
    stopCamera()
    showPhoto(blob)
  ...
  return (
    <section className="input-method-panel camera-panel">
      <Camera size={29} />
      <h2>{t('Scan with your camera')}</h2>
      <p>{t('Point your camera at a payslip, receipt or purchase record. Lay it flat in good light and fill the frame.')}</p>

      <div className="camera-frame" style={liveOn && videoRatio ? { aspectRatio: videoRatio } : undefined}>
        <video ref={videoRef} className={liveOn ? '' : 'hidden'} playsInline muted aria-label={t('Camera preview')} />
        {liveOn && <div className="camera-guide" aria-hidden="true" />}
        {photo && <img src={photo.url} alt={t('Your captured document')} />}
        {!liveOn && !photo && (
          <div className="camera-placeholder">
            {liveSupported ? <Camera size={34} /> : <CameraOff size={34} />}
            <span>{liveSupported ? t('Your camera preview will appear here.') : t('Live preview needs a secure (https) page. Use your camera app below.')}</span>
          </div>
        )}
      </div>
        ...
        {photo ? (
          <>
            ...
            <button type="button" className="primary-button" onClick={() => onPhoto(photo.blob)} disabled={busy}>
              ...
              {busy ? t('Reading…') : t('Read photo')}
            </button>
          </>
        ) : liveOn ? (
          <>
            <button type="button" className="secondary-button" onClick={stopCamera}>{t('Cancel')}</button>
            <button type="button" className="primary-button" onClick={takePhoto}><Camera size={18} />{t('Take photo')}</button>
          </>
        ) : liveSupported ? (
          <button type="button" className="primary-button" onClick={startCamera} disabled={starting || busy}>
            ...
            {starting ? t('Starting camera…') : t('Start camera')}
          </button>
        ) : null}
      </div>
      ...
      {!liveOn && (
        <label className={`camera-app-link ${busy ? 'disabled' : ''}`}>
          <Smartphone size={17} />
          {photo ? t('Retake with your camera app') : liveSupported ? t('Or use your phone’s camera app') : t('Open your camera app')}
